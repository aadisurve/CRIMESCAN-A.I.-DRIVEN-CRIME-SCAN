...
class Criminal:
    def __init__(self,root):
        ...
        # top image 1
        img = Image.open(r"C:\Users\aadis\OneDrive\Desktop\criminal recognition system\images\black.jpg")
        ...
        # top image 2
        img1 = Image.open(r"C:\Users\aadis\OneDrive\Desktop\criminal recognition system\images\black.jpg")
        ...
        #top image 3
        img2 = Image.open(r"C:\\Users\\aadis\\OneDrive\\Desktop\\criminal recognition system\\images\\black1.jpg")
        ...
        #bg image
        img3 = Image.open(r"C:\\Users\\aadis\\OneDrive\\Desktop\\criminal recognition system\\images\\bg.jpg")
        ...
        img_left = Image.open(r"C:\\Users\\aadis\\OneDrive\\Desktop\\criminal recognition system\\images\\black1.jpg")
        ...
        img_right = Image.open(r"C:\\Users\\aadis\\OneDrive\\Desktop\\criminal recognition system\\images\\black1.jpg")
    ...
    def add_data(self):
        if self.var_station.get() == "Select Station" or self.var_adhaar.get() == "":
            ...
            try:
                conn = mysql.connector.connect(host="localhost", user="root", password="AadiSurve7654", database="face_recognizer")
    ...
    def fetch_data(self):
        ...
        conn = mysql.connector.connect(
            host="localhost",
            user="root",
            password="AadiSurve7654",
            database="face_recognizer"
        )
        my_cursor = conn.cursor()

        # SELECT fields in the order expected by get_cursor
        my_cursor.execute("""
            SELECT 
                station, pin, stationID, officerBatch,
                adhaar, name, city, state, gender, dob,
                email, phone, address, crime, photoSample
            FROM criminal
        """)
    ...
    def get_cursor(self, event=""):
        ...
        # Ensure 15 values are received
        if data and len(data) == 15:
            self.var_station.set(data[0])
            self.var_pin.set(data[1])
            self.var_stationID.set(data[2])
            self.var_officerBatch.set(data[3])
            
            self.var_adhaar.set(data[4])
            ...
            self.var_crime.set(data[13])
            
            self.var_radio1.set(data[14])  # This is the 'photoSample' column
        ...
        if self.var_station.get() == "Select Station" or self.var_adhaar.get() == "":
            ...
            try:
                ...
                if Update > 0:
                    ...
                    my_cursor.execute("""
                        UPDATE criminal SET 
                            name=%s, city=%s, state=%s, gender=%s, dob=%s, email=%s, phone=%s, 
                            address=%s, crime=%s, station=%s, pin=%s, stationID=%s, 
                            officerBatch=%s, photoSample=%s 
                        WHERE adhaar=%s
                    """, (
                        self.var_name.get(),
                        self.var_city.get(),
                        ...
                        self.var_stationID.get(),
                        self.var_officerBatch.get(),
                        self.var_radio1.get(),  # assuming photoSample is from a radio button variable
                        self.var_adhaar.get()
                    ))
                    ...
                    self.fetch_data()  # make sure this is a function call
                else:
                    return
            except Exception as es:
                messagebox.showerror("Error", f"Due To: {str(es)}", parent=self.root)
